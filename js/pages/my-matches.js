...
const MyMatchesPage = (() => {
  ...
  async function loadMatches(status) {
    ...
    try {
      ...
      list.innerHTML = `<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-title">Failed to load matches</div><div class="empty-desc">${Utils.escapeHtml(e.message||'')}</div></div>`;
    }
  }
